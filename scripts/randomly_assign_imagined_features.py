...
if __name__ == "__main__":
    ...
    for f in competencies:
        ...
        remove_items(data['dependencies'], ['imagined.feature.current',
                     'imagined.feature.future', 'imagined.feature.unlikely'])
        random_shuffle(hash[0], 10) and data['dependencies'].append(
            'imagined.feature.current')
        random_shuffle(hash[2], 5) and data['dependencies'].append(
            'imagined.feature.future')
        random_shuffle(hash[3], 2) and data['dependencies'].append(
            'imagined.feature.unlikely')
